...
#
# Purpose:      student_tracking Demo. Demonstrating OOP, Tkinter GUI module,
...
import drill50_student_tracking_main
import drill50_student_tracking_gui
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_student_tracking (col_fname,col_lname,col_fullname,col_phone,col_current_course) VALUES (?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@current_course.com'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_current_course FROM tbl_student_tracking WHERE col_fullname = (?)""", [value])
        ...
        for data in varBody:
            ...
            self.txt_current_course.insert(0,data[3])
...
def addToList(self):
    ...
    if not "@" or not "." in var_current_course: # will use this soon
        print("Incorrect current_course format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_current_course) > 0): # enforce the user to provide both names
        conn = sqlite3.connect('db_student_tracking.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_student_tracking WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_student_tracking (col_fname,col_lname,col_fullname,col_phone,col_current_course) VALUES (?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_current_course))
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be alowed to update changes for phone and current_courses.
    ...
    var_current_course = self.txt_current_course.get().strip()
    if (len(var_phone) > 0) and (len(var_current_course) > 0): # ensure that there is data present
        conn = sqlite3.connect('db_student_tracking.db')
        with conn:
            cur = conn.cursor()
            ...
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_current_course) FROM tbl_student_tracking WHERE col_current_course = '{}'""".format(var_current_course))
            count2 = cur.fetchone()[0]
            print(count2)
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone,var_current_course,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('db_student_tracking.db')
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student_tracking SET col_phone = '{0}',col_current_course = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_current_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone, var_current_course))
            onClear(self)
        conn.close()
    else:
        messagebox.showerror("Missing information","Please select a name from the list. \nThen edit the phone or current_course information.")
    onClear(self)
